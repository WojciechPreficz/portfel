...
const chartOption = computed(() => ({
  ...
  xAxis: { type: "category", boundaryGap: false, data: props.historyDates, axisLabel: { color: "#7b817b", formatter: (value: string) => value.slice(5) }, axisLine: { lineStyle: { color: "#d8ddd6" } } },
  ...
  series: [{ type: "line", smooth: true, symbol: "none", data: props.historyValues, lineStyle: { width: 3, color: "#c2ec62" }, areaStyle: { color: "rgba(194, 236, 98, .18)" } }],
}));
...
<template>
  <section class="content-grid">
    ...
    <div class="metric-card panel"><p class="panel-kicker">XIRR ROCZNY</p><div v-if="loading" class="skeleton skeleton-lg"></div><strong v-else :class="isPositive(summary?.xirr_pct) ? 'positive' : 'negative'">{{ formatPercent(summary?.xirr_pct) }}</strong><p class="metric-sub">Zwrot ważony czasem</p></div>
